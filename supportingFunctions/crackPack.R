crackPack <- 
  function (spoiler, packsPerMythic = 8, packsPerFoil = 6, foilsInSet = T) 
  {
    ...
    if (nrow(mythics) > 0) {
      mythicCheck <- sample(c(0, 1), 1, prob = c(1 - (1/packsPerMythic), 
                                                 1/packsPerMythic))
      if (mythicCheck == 0) {
    ...
    if (foilsInSet == T) {
      foilCheck <- sample(c(0, 1), 1, prob = c(1 - (1/packsPerFoil), 
                                               1/packsPerFoil))
      if (foilCheck == 1) {
